  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="bg-black text-white min-h-[90vh] flex flex-col items-center justify-center text-center px-8 relative overflow-hidden">
        ...
        <p className="text-xs tracking-[0.4em] text-yellow-400 uppercase mb-4">Handcrafted in Nepal</p>
        <h1 className="text-6xl md:text-8xl font-bold tracking-widest mb-6">MARYURIKA</h1>
        ...
        {goldRate && (
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-8 text-xs text-gray-400">
      ...
      <section className="bg-gray-50 py-16 px-8">
        <div className="max-w-5xl mx-auto">
          <h2 className="text-2xl font-bold tracking-widest uppercase text-center mb-10">Shop by Category</h2>
          <div className="flex gap-4 justify-center flex-wrap">
            {categories.map(cat => (
              <Link key={cat.label} to={cat.path}
                className="flex flex-col items-center gap-3 bg-white rounded-xl px-8 py-6 shadow-sm hover:shadow-md transition group border border-transparent hover:border-black">
                <span className="text-3xl">{cat.emoji}</span>
                <span className="text-sm font-semibold uppercase tracking-wider group-hover:text-black">{cat.label}</span>
              </Link>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 px-8">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-center mb-8">
            <h2 className="text-2xl font-bold tracking-widest uppercase">Featured Collection</h2>
            <Link to="/search" className="text-sm text-gray-400 hover:text-black underline">View all</Link>
          </div>

          {loading ? (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {[...Array(8)].map((_, i) => <div key={i} className="bg-gray-100 rounded-xl h-72 animate-pulse" />)}
            </div>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
      ...
      <section className="bg-black text-white py-20 px-8 text-center">
        ...
        <button onClick={() => navigate('/search')}
          className="bg-yellow-400 text-black px-8 py-3 font-semibold text-sm rounded hover:bg-yellow-300 transition">
          Explore Collection
        </button>
      </section>
      ...
      <section className="py-16 px-8 bg-gray-50">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-2xl font-bold tracking-widest uppercase text-center mb-12">Why Maryurika</h2>
      ...
      {/* WhatsApp CTA */}
      <section className="py-12 px-8 bg-green-50 text-center">
        <p className="text-sm text-gray-600 mb-3">Have a custom design in mind?</p>
